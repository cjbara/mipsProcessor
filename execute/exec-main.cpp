...
/* This procedure runs the machine as if there is no next instruction. 
   The simple machine only works on one instruction at a time.
   This will wait to see where the branch takes you before getting a new instruction.
*/
...
/* This procedure works on two instructions at a time. It fetches one instruction 
   and keeps it on the queue for the next loop through. On each loop it fecthes one
   instruction and then executes the instruction fetched in the previous loop
   Even if the instruction is a branch instruction, the next instruction will be 
...
/* This procedure prints out statistics for the processor 
   into a file called stats.csv
   These stats include the number of each specific instruction type
*/
void outputStats()
{
  ofstream statFile;
...
/* This function prints out the options you get when executing the file */ 
void printHelp()
{
